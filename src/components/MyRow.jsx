...
export const MyRow = (props) => {
    ...
    const renderRows => (props) => {
        ...
        for (let row = 0; row < getTotalRows(props.rawData); row++) {
            if (row == 0) {
                rows +=
                    <tr>
                        <td className="all-table-borders" rowSpan={getTotalRows(props.rawData)}><div style={marginStyle}><MyPill text={props.rowData['id']} type="light-grey-pill"/></div></td>
                        <td className="all-table-borders" rowSpan={getTotalRows(props.rawData)}><div style={marginStyle}><MyPill text={"↑ " + props.rowData['Priority']}/></div></td>
                        <td className="all-table-borders" rowSpan={getTotalRows(props.rawData)}><div style={marginStyle}>{props.rowData['Group']}</div></td>
                        <td className="all-table-borders" rowSpan={getNumberOfRowsPerProduct(props.rawData, props.rowData['Products'][0]['Product Name'])}><div style={marginStyle}>{product['Product Name']}</div></td>
                ...
                <tr>
                    <td className="all-table-borders">{
                            props.rowData['Products'].map((product, idx) => {
                                return (
                                    <td id={"pn-" + props.index + "-" + idx} className={(idx < props.rowData['Products'].length - 1) && "products-column"} rowSpan={getNumberOfRowsPerProduct(props.rawData, product['Product Name'])}><div style={marginStyle}>{product['Product Name']}</div></td>
                                ); 
                            })
                        }
                    </td>
